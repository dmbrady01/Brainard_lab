%To get a calculation of transition probabilites with more than 2
%branch points over multiple days.


%% Asks for basic parameters

multiple_trans.parameters.computer = pwd;

slashes = strfind(multiple_trans.parameters.computer,'/');
multiple_trans.parameters.bird_name = multiple_trans.parameters.computer(slashes(end)+1:end);

%Asks to add a modifying phrase
multiple_trans.parameters.phrase = input('Phrase for song folder search?  ','s');
if ~isempty(strfind(multiple_trans.parameters.phrase,'''')) == 1
    multiple_trans.parameters.phrase = [];
end

%creates a list of song folders
omit = 0;
db_list_song_folders(multiple_trans.parameters.phrase,omit,multiple_trans.parameters.computer);
...
%asks for second
multiple_trans.parameters.phrase2 = input('Phrase to append onto created folders? ','s');
if isempty(multiple_trans.parameters.phrase2) == 0
    multiple_trans.parameters.phrase2 = ['_' multiple_trans.parameters.phrase2];
end
...
%% Sees if program was run on this bird previously

if exist([multiple_trans.parameters.computer '/all_days_sequence_' multiple_trans.parameters.phrase multiple_trans.parameters.phrase2],'dir') == 0
    mkdir([multiple_trans.parameters.computer '/all_days_sequence_' multiple_trans.parameters.phrase multiple_trans.parameters.phrase2])
end

all_days_contents = db_list_folder_contents([multiple_trans.parameters.computer '/all_days_sequence_' multiple_trans.parameters.phrase multiple_trans.parameters.phrase2]);
load([multiple_trans.parameters.computer '/song_folders_' multiple_trans.parameters.phrase '.mat'])
fid = fopen([multiple_trans.parameters.computer '/song_folders_' multiple_trans.parameters.phrase '.txt'],'r');

for iii = 1:length(song_folders)
    current_line = fgetl(fid);
    if sum(strcmp(all_days_contents,[multiple_trans.parameters.bird_name ...
        '_' song_folders{iii} '_' multiple_trans.parameters.con_or_div '.mat'])) > 0
    else
        cd([multiple_trans.parameters.computer '/' current_line])

        % Make a directory to store figures and variables
        mkdir(['multi_sequence_' song_folders{iii} '_' multiple_trans.parameters.con_or_div '_' multiple_trans.parameters.phrase multiple_trans.parameters.phrase2])
        ...
        for i  = 1:length(multiple_trans.syl_order)
            ...
            for j = 1:multiple_trans.parameters.num_tp
                ...
                elseif strcmpi(multiple_trans.parameters.con_or_div, 'con')
                    temp_syl_order = [temp_syl_order...
                        strfind(multiple_trans.syl_order{i}, multiple_trans.parameters.tpofinterest{j})];
                end
            end
        ...
        for i = 1:length(multiple_trans.parameters.tpofinterest)
            ...
            db_prepare_bootstrp_hist(...
                ...
                ' on ' song_folders{iii} '  (bootstrap' num2str(multiple_trans.parameters.numbtstrp_trials) ' trials)'],...
                'Relative Frequency',...
            ...
            saveas(figure(gcf), ['multi_sequence_' song_folders{iii} '_' multiple_trans.parameters.con_or_div '_'...
                multiple_trans.parameters.phrase multiple_trans.parameters.phrase2 ...
                '/Transition_probability_' multiple_trans.parameters.tpofinterest{i} '_' song_folders{iii}], 'fig')
        ...
        title(['Tranisition probability of each song throughout ' song_folders{iii}])
        xlabel('Song Number')
        ylabel('Transition probability')
        ylim([-0.1 1.1])
        saveas(figure(gcf), ['multi_sequence_' song_folders{iii} '_' multiple_trans.parameters.con_or_div '_'...
            multiple_trans.parameters.phrase multiple_trans.parameters.phrase2 ...
            '/Trans_prob_per_song_' song_folders{iii}], 'fig')
        ...
        legend(multiple_trans.parameters.tpofinterest)
        title(['Cumulative tranisition probability on ' song_folders{iii}])
        xlabel('Syllable Number')
        ylabel('Transition probability')
        ylim([-0.1 1.1])
        saveas(figure(gcf), ['multi_sequence_' song_folders{iii} '_' multiple_trans.parameters.con_or_div '_'...
            multiple_trans.parameters.phrase multiple_trans.parameters.phrase2 ...
            '/Cumul_trans_prob_' song_folders{iii}], 'fig')
        ...
        legend(multiple_trans.parameters.tpofinterest)
        title(['Running average tranisition probability on ' song_folders{iii}...
        ...
        saveas(figure(gcf), ['multi_sequence_'  song_folders{iii} '_' multiple_trans.parameters.con_or_div '_'...
            multiple_trans.parameters.phrase multiple_trans.parameters.phrase2 ...
            '/Run_avg_trans_prob_' song_folders{iii}], 'fig')
        ...
        db_prepare_bootstrp_hist(...
            ...
            song_folders{iii} '  (bootstrap' num2str(multiple_trans.parameters.numbtstrp_trials) ' trials)'],...
            'Relative Frequency',...
        ...
        saveas(figure(gcf), ['multi_sequence_' song_folders{iii} '_' multiple_trans.parameters.con_or_div '_' ...
            multiple_trans.parameters.phrase multiple_trans.parameters.phrase2 '/Entropy_'...
            multiple_trans.parameters.bird_name '_' song_folders{iii}], 'fig')
        ...
        for i = 1:length(multiple_trans.parameters.tpofinterest)
            ...
            saveas(figure(gcf), ['multi_sequence_'  song_folders{iii} '_' multiple_trans.parameters.con_or_div '_'...
                multiple_trans.parameters.phrase multiple_trans.parameters.phrase2 '/History_dependence_'...
                multiple_trans.parameters.tpofinterest{i} '_' song_folders{iii}], 'fig')
        ...
        %         ' for ' multiple_trans.parameters.tpofinterest{i} ' on ' song_folders{iii}...
        %         '    (across song shuffle)'])
        %     xlabel(['History dependence (ex: | p(ab|ab) - p(ab|ac) - p(ab|ad) |)'])
        %
        %     %saves figure
        %     saveas(figure(1234567+i), ['multi_sequence_' ...
        %         '/Is_History_Dependent_(across_shuffle)_' multiple_trans.parameters.tpofinterest{i} '_' song_folders{iii}], 'fig')
        % end
        ...
        for i = 1:length(multiple_trans.parameters.tpofinterest)
            ...
            saveas(figure(gcf), ['multi_sequence_' song_folders{iii} '_' multiple_trans.parameters.con_or_div '_'...
                multiple_trans.parameters.phrase multiple_trans.parameters.phrase2 ...
                '/Is_History_Dependent_(within_song_shuffle)_' multiple_trans.parameters.tpofinterest{i} '_' song_folders{iii}], 'fig')
        end
        ...
        save([multiple_trans.parameters.computer...
            '/all_days_sequence_' multiple_trans.parameters.phrase multiple_trans.parameters.phrase2 '/' multiple_trans.parameters.bird_name '_' ...
            song_folders{iii} '_' multiple_trans.parameters.con_or_div '.mat'], 'multiple_trans')
        
    end
    close all
    mult_fields = fieldnames(multiple_trans);
    for i = 1:length(mult_fields)
        if strcmpi(mult_fields{i},'parameters') == 1
        else
            multiple_trans = rmfield(multiple_trans,mult_fields{i});
        end
    end
    cd(multiple_trans.parameters.computer)
end
...
% save(['multi_sequence_'  '/multi_sequence_' ])

% %move batch files to new folder, make sure the batch file name is not too
% %similar to the name of your bird
% if strcmpi(input(['Move batch files to multi_sequence_'  ' folder? (y or n)  '], 's'),'y')
%     movefile([multiple_trans.parameters.batchfile(1:4) '*'],['multi_sequence_' ])
% else
% end
