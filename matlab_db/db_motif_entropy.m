function [ syl_we absolute_time ] = db_motif_entropy( cbin_not_mat_file, motif)
...
for j = 1:length(motif)
    ...
    if isempty(absolute_time.(motif(j))) == 1
        ...
        if isempty(strfind(cbin_not_mat_file, '.not.mat'))
            [spec time] = db_spec_syllable(cbin_not_mat_file,motif(j),.2,.2,motif(1:j-1),motif(j+1:end));
        elseif ~isempty(strfind(cbin_not_mat_file, '.not.mat'))
            [spec time] = db_spec_syllable(cbin_not_mat_file(1:end-8),motif(j),.2,.2,motif(1:j-1),motif(j+1:end));
        end
